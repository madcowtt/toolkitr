test_that("decile summary matches expected group stats", {
  ...
  summary <- summary %>% dplyr::mutate(
    pct_of_total = as.numeric(sprintf("%.3f", .data[["sum_market_decile_nbrx_pre"]] / sum(toolkitr::profile_example$market_decile_nbrx_pre, na.rm = TRUE)))
  )
  ...
  expected <- tibble::tibble(
    ...
    pct_of_total = c(0, 0.329, 0.199, 0.472, 1)
  )
